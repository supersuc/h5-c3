    ...
    function getsuggest(){
        $.ajax({
            ...
            success:function(res){
                if (res.code === 0){
                    var suggetData = res.data;
                    var suggest = document.getElementById("suggest");
                    var str ="";
                    for (var i=0;i<suggetData.length;i++){
                        str +=`<li class="suggest-link">${suggetData[i].name}</li>`
...
window.onscroll = function(){
  ...
  function scrollShowBtn(){
    ...
    if(top > 300) {
      document.getElementById("topsnav").style.display = "block";
    }else{
      document.getElementById("topsnav").style.display = "none";
    }
  }
